...
### 팀원 회고
- **김가영**:
  >이번 프로젝트를 통해 처음으로 프론트엔드에서 사용자 화면을 직접 구성하고, 백엔드 API와 연결하는 과정을 경험했습니다. 단순히 화면을 만드는 것을 넘어서, 사용자의 입력이 어떤 식으로 처리되고 서버와 데이터를 주고받는지 전체 흐름을 이해할 수 있었습니다. 처음 구성하는 화면에 막막함을 크게 느끼고 figma를 통한 화면설계서 제작과정에 정말 많은 시간을 쏟아부으며 기본적인 틀을 잡아가며 익숙해지려 노력했습니다.
특히 상태 관리와 API 연동을 직접 다뤄보면서, 화면에 보이는 요소 뒤에서 어떤 로직이 동작하는지에 대해 많이 고민하게 되었습니다. 작은 기능 하나라도 자연스럽게 동작하도록 만들기 위해 조건 처리나 예외 상황을 신경 써야 했고, 이를 통해 사용자 경험을 고려한 개발의 중요성을 느낄 수 있었습니다.
처음에는 화면 구현에만 집중했지만, 점점 구조를 생각하면서 코드를 작성하게 되었고, 그 과정에서 컴포넌트 분리나 코드 재사용성의 필요성도 체감할 수 있었습니다. 이번 경험을 통해 프론트엔드는 단순히 디자인을 구현하는 영역이 아니라, 서비스 흐름을 만들어가는 중요한 역할이라는 점을 이해하게 되었습니다.
  <br>
  
- **김다솜**:
  >프론트 작업을 하면서 구조를 먼저 설계하는 것이 중요하다는 것을 알게되었습니다. 이번 프로젝트를 통해 첫 프론트 작업을 하였습는데 처음 하는거라 쉽지 않았습니다.  한 화면 안에서도 여러 기능과 API가 함께 연결되어 있어 부모 컴포넌트와 자식 컴포넌트의 역할이 명확하지 않으니 상태 관리와 데이터 흐름이 꼬이곤 하였습니다. 실제로 구현 과정에서도 어떤 데이터는 부모에서 관리하고 어떤 동작은 자식에서 처리할지 정리가 되지 않아 수정 범위가 계속 커졌고, 백엔드와 연동할 때도 응답 구조와 화면 구조가 맞지 않아 어려움을 겪었습니다. 이를 통해 프론트엔드 개발에서도 단순히 UI를 나누는 것이 아니라, 컴포넌트의 책임과 상태의 흐름을 먼저 설계하는 과정이 중요하다는 점을 배울 수 있었습다.
이번 프로젝트를 통해 프론트엔드에서도 화면 구현 능력을 향상 시킬 수 있었고, 컴포넌트 책임과 상태 흐름을 먼저 설계하는 과정이 중요하다는 점을 배웠습니다.
<br>
...
- **이서윤**:
  >프론트엔드 개발에서 가장 어려웠던 부분은 백엔드 API와의 연동이었습니다.
초기에는 목업 데이터로 화면을 구성하다 보니 실제 API 응답 구조와 맞지 않는 경우가 많았고, 데이터 구조를 이해해서 화면에 맞게 가공하는 과정이 어려웠습니다다.
특히 응답 데이터의 필드명이나 구조가 예상과 다를 때 오류가 발생했고, 이를 파악하는 데 시간이 많이 걸렸습니다.
이번 프로젝트를 통해 프론트엔드는 단순히 UI를 만드는 것이 아니라 백엔드와의 데이터 흐름을 이해하는 것이 중요하다는 걸 느꼈습니다.
또 API 명세서를 보고 데이터 구조를 파악하는 능력이나, 상태 관리를 통해 화면에 반영하는 방식에 따라 구현 결과가 달라진다는 것도 알게 되었습니다. 다음에는 이 부분을 더 신경써서 구현하겠습니다.
